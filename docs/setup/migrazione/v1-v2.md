...
!!! warning "Attenzione"
    Poichè i log delle richieste sono collegati ai ContentType, è necessario sovrascrivere i valori creati da Django nella migrazione iniziale per la consistenza del backup da importare.
...
!!! warning "Attenzione"
    Questo campo è presente e viene salvato automaticamente nella nuova release quando una richiesta viene presa in carico la prima volta. 

Deve essere inizializzato per tutte le richieste con la data della prima presa in carico. Questo è necessairio solo nel processo di migrazione dalla v1.x alla v2 x.  
