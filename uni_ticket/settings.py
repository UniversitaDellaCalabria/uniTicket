...
TICKET_MESSAGES_ATTACHMENT_SUBFOLDER = getattr(settings, "TICKET_MESSAGES_ATTACHMENT_SUBFOLDER", 'messages')
TICKET_TASK_ATTACHMENT_SUBFOLDER = getattr(settings, "TICKET_TASK_ATTACHMENT_SUBFOLDER", 'task')
CATEGORY_CONDITIONS_ATTACHMENT_SUBFOLDER = getattr(settings, "CATEGORY_CONDITIONS_ATTACHMENT_SUBFOLDER", 'conditions')

UNITICKET_JWE_RSA_KEY_PATH = getattr(
    settings, 
    "UNITICKET_JWE_RSA_KEY_PATH", 
    'saml2_sp/saml2_config/certificates/key.pem'
)
UNITICKET_JWE_ALG = getattr(
    settings, 
    "UNITICKET_JWE_ALG", 
    "RSA-OAEP"
)
UNITICKET_JWE_ENC = getattr(
    settings, 
    "UNITICKET_JWE_ENC", 
    "A128CBC-HS256"
)
...
TICKET_CONDITIONS_FIELD_ID = getattr(settings, "TICKET_CONDITIONS_FIELD_ID", 'condizioni_field_id')
...
TICKET_DESCRIPTION_LABEL =getattr(
    settings, "TICKET_DESCRIPTION_LABEL",
     _('Descrizione')
)
TICKET_DESCRIPTION_HELP_TEXT = getattr(
    settings, "TICKET_DESCRIPTION_HELP_TEXT",
    _(
...
TICKET_GENERATE_URL_BUTTON_NAME  = getattr(
    settings, "TICKET_GENERATE_URL_BUTTON_NAME",
    'generate_url_submit'
)
...
TICKET_INPUT_MODULE_NAME = getattr(
    settings, "TICKET_INPUT_MODULE_NAME",
    'ticket_input_module'
)
...
# If 0 = Unlimited
MAX_DAILY_TICKET_PER_USER = getattr(
    settings, "MAX_DAILY_TICKET_PER_USER",
    10
)

# user contexts
CONTEXT_SIMPLE_USER = getattr(
    settings, "CONTEXT_SIMPLE_USER",
    _('Utente')
)

# To change the URLs prefix for every user type
MANAGER_PREFIX = getattr(
    settings, "MANAGER_PREFIX",
    'Manager'
)
OPERATOR_PREFIX = getattr(
...
NO_MORE_COMPETENCE_OVER_TICKET = getattr(
    settings, "NO_MORE_COMPETENCE_OVER_TICKET",
    _("Nessuna competenza sulla richiesta")
)
...
TICKET_MIN_DIGITS_TO_COMPRESS = getattr(
    settings, "TICKET_MIN_DIGITS_TO_COMPRESS",
    90
)
...
EMPLOYEE_ATTRIBUTE_LABEL = getattr(
    settings, "EMPLOYEE_ATTRIBUTE_LABEL",
    'Matricola dipendente'
)

# Label
ORGANIZATION_EMPLOYEE_LABEL = getattr(
    settings, "ORGANIZATION_EMPLOYEE_LABEL",
    'Dipendenti'
)
...
TICKET_UPDATED = getattr(
    settings, "TICKET_UPDATED",
    _("""La richiesta "{ticket}" è stata aggiornata con il seguente messaggio:

{message}""")
)

USER_TICKET_MESSAGE = getattr(
    settings, "USER_TICKET_MESSAGE",
    _("""Hai {status} un nuovo messaggio per la richiesta \"{ticket}\"
...
TICKET_DELETED = getattr(
    settings, "TICKET_DELETED",
    _("""La richiesta "{ticket}" è stata eliminata correttamente.""")
)

SUMMARY_USER_EMAIL = getattr(
    settings, "SUMMARY_USER_EMAIL",
    _("""La seguente richiesta {event msg}:

{ticket}""")
)

SUMMARY_EMPLOYEE_EMAIL = getattr(
    settings, "SUMMARY_EMPLOYEE_EMAIL",
    _("""Ci sono {opened_ticket_number} richieste da gestire.
...
NEW_TICKET_CREATED_EMPLOYEE_BODY = getattr(
    ...
    _("""E' stata effettuata una nuova richiesta all'ufficio {destination_office}.
...
NEW_MESSAGE_RECEIVED_EMPLOYEE_BODY = getattr(
    settings, "NEW_MESSAGE_RECEIVED_EMPLOYEE_BODY",
    _("""Hai ricevuto un nuovo messaggio per la richiesta \"{ticket}\"
...
NEW_TICKET_ASSIGNED_TO_OPERATOR_BODY = getattr(
    settings, "NEW_TICKET_ASSIGNED_TO_OPERATOR_BODY",
    _("""Ti è stata assegnata una nuova richiesta da {manager}.
...
NEW_TICKET_UPDATE_OLD_EN = getattr(
    settings, "NEW_TICKET_UPDATE_OLD_EN",
    _(
        "Dear user,"
...
USER_TICKET_MESSAGE_OLD_EN = getattr(
    settings, "USER_TICKET_MESSAGE_OLD_EN",
    _(
        "Dear {user},"
...
TICKET_UPDATED_OLD_EN = getattr(
    settings, "TICKET_UPDATED_OLD_EN",
    _(
        "Dear {user},"
        "the ticket:"
        ""
...
SUMMARY_USER_EMAIL_OLD_EN = getattr(
    settings, "SUMMARY_USER_EMAIL_OLD_EN",
    _(
        "Dear {user},"
...
SUMMARY_EMPLOYEE_EMAIL_OLD_EN = getattr(
    settings, "SUMMARY_EMPLOYEE_EMAIL_OLD_EN",
    _(
        "Dear {user},"
