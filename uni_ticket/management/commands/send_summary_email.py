...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        users = OrganizationalStructureOfficeEmployee.objects.filter(employee__is_active=True,
                                                                     employee__email_notify=True).values_list('employee')
        status = send_summary_email(users=[get_user_model().objects.get(pk=user[0]) for user in set(users)])
